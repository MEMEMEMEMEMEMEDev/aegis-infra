...
round, the edge, the projects, the traffic, the capacity, the last
builds and the off-site copies, and draws them as Projects,
Deployments, Domains, Traffic, Storage, Security, Machine and Health,
with **four states** rather than two: a source that could not be
reached is «could not look», never green. It is on loopback
(`ssh -L 7391:127.0.0.1:7391` from elsewhere) and it is the fastest way
to see which of the commands below is worth running. See
