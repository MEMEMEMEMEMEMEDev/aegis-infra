...
# ── the wizard ──────────────────────────────────────────────────────
config_wizard() {
    printf '\n\033[1m════ GUIDED CONFIGURATION OF AEGIS ════\033[0m\n'
    ...
    ask PLATFORM_REPO "aegis-platform" _v_reponame \
      "PLATFORM repo that the init CREATES under that owner (GitOps" \
      "reads from here). The init OWNS it: it writes commits, tags," \
      "settings and webhooks to it — name one that does not exist" \
      "yet, never a repo you already use for something else."

    ask APP_REPO "aegis-canary" _v_reponame \
      "Repo of the canary APP that the init CREATES and seeds (CI" \
      "writes builds to it and commits the digest of every deploy)." \
      "Same rule: a new name, not an existing repo."
    ...
    ask ACME_EMAIL "$git_email" _v_email \
      "Contact email of whoever runs this instance. Under cloudflare" \
      "it is also the email of the Cloudflare account when the API" \
      "credential is a Global API Key. Under local nothing is sent" \
      "to it; it is recorded and that is all."
...
ensure_config() {
    if [[ "${FORCE_CONFIGURE:-false}" == "true" || ! -f "$CONF_FILE" ]]; then
        ...
        ni_mode && die "--non-interactive without $CONF_FILE — the wizard does not run unattended; pre-create the conf (run 'aegis init --configure' once with a terminal, or write it by hand from init/aegis-init.conf.example)"
