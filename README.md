...
| same host | two tenants signed up from their contracts, their data restored from backups, catalogue served over HTTPS | done; 12 products, photos, orders and users where the backup said they would be |
...
- A GitHub account. The init **creates and owns** the two repos it
  needs (the platform repo and a canary) through `gh`; it needs a
