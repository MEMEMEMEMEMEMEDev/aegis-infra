...
# Los números viven en platform/planes.yaml y se pueden reajustar para
...
| `k8s/base/ai-system/ruteo.yaml` | el mapa organización → plan de AI |
...
`platform/ai/ruteo.yaml`, plano de plataforma, fuera del alcance de las
organizaciones:
...
**Solo se declara lo que se sabe servir.** `ai/ruteo.yaml` no lleva
...
```
ai/ruteo.yaml   ─┐
platform/planes.yaml ─┼─→  k8s/base/ai-system/ruteo.yaml  (ConfigMap ai-ruteo)
...
|---|---|---|
| `ai/ruteo.yaml` | `capacidades` | plataforma |
| `planes.yaml` | `planes` | plataforma |
...
- No inventes valores de plan ni de cuota. Si el que hace falta no
  existe, el cambio es agregar un plan en `platform/planes.yaml` y
  decirlo.
