...
# vlogs-events: aceptado — es historia de bootstraps, se deduplica
...
# ── 85.2 TRAER DE LA SEED lo que la instancia no tenga ──────────
...
# ── 85.11 ingesta del histórico del init a vlogs-events ───────────
...
VLE_IP="$(_svc_ip vlogs-events)" \
    || die "vlogs-events sin IP alcanzable (¿pod sin Ready? kubectl -n observability get endpoints vlogs-events)"
_ingesta_historico() {
...
if (( N_GATES > 0 )); then
    ...
    if (( N_YA >= N_GATES )); then
        log_info "histórico ya en vlogs-events ($N_YA filas >= $N_GATES del archivo): no se re-ingesta"
    else
        run_cmd retry_net 3 _ingesta_historico || \
            die "ingesta de gates.jsonl a vlogs-events FALLÓ — el endpoint DEBE existir (fase-85 §6.11); revisar la App vlogs-events"
        log_ok "histórico ingestado: $N_GATES líneas de gates.jsonl → vlogs-events (la historia sobrevive a --reset-state)"
    fi
else
    log_warn "sin gates.jsonl que ingestar (¿--reset-state recién?) — lo ya ingestado en corridas previas SOBREVIVE en vlogs-events"
fi
