...
# semilla/canario/k8s/base/ingressroute.yaml, y con #54 la ruta se mudó
...
D49=""
SEED_BASE="$AEGIS_ROOT/semilla/canario/k8s/base"
...
CR="$P/k8s/organizations/org-canary/ruteo.yaml"
if [[ -f "$CR" ]]; then
    CR_NC="$(nc "$CR")"
    ...
    D49="$D49 falta k8s/organizations/org-canary/ruteo.yaml: el canario no tiene ruta en NINGÚN lado;"
fi
# (c) y está entregada: listada en el kustomization de la organización.
grep -qE '^\s*-\s*ruteo\.yaml' "$P/k8s/organizations/org-canary/kustomization.yaml" \
    || D49="$D49 ruteo.yaml no listado en el kustomization de org-canary (A19: el archivo existe y no lo aplica nadie);"
...
else pass "la ruta del canary la pone la plataforma (org-canary/ruteo.yaml, entregada), su repo no puede escribirla y el AppProject lo impide"; fi
}
