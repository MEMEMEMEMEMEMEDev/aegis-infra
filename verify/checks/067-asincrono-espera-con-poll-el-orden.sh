# titulo: asíncrono espera con poll; el orden respeta la causa (clase D)
# origen: verify-static.sh (v2) ══ 67
check() {
D67=""
NC80="$(sed -e ':a' -e '/\\$/{N;s/\\\n/ /;ba}' "$FASES/80-supply-chain.sh" | nc)"
...
L_SYNC="$(grep -n 'argo_sync kyverno-policies' "$FASES/80-supply-chain.sh" | head -1 | cut -d: -f1)"
L_WH="$(grep -n '"webhooks-scopeados"' "$FASES/80-supply-chain.sh" | head -1 | cut -d: -f1)"
...
L_IDX="$(grep -n '"job-main-indexado"' "$FASES/60-webhook.sh" | head -1 | cut -d: -f1)"
L_NEXT="$(grep -n 'NEXT_MB=' "$FASES/60-webhook.sh" | head -1 | cut -d: -f1)"
if [[ -z "$L_IDX" || -z "$L_NEXT" ]] || (( L_IDX > L_NEXT )); then
    D67="$D67 la 60 captura NEXT_MB sin esperar el branch indexing;"
fi
nc "$FASES/60-webhook.sh" | grep -q 'deliveries/\$did/attempts' \
