# titulo: retome de la 80 NO re-buildea si ya hay firma válida (P1.8 in-VM)
...
L_SKIP="$(grep -in 'docker-content-digest' "$FASES/80-supply-chain.sh" | head -1 | cut -d: -f1)"
L_BLD="$(grep -n 'jenkins_next_build hello-aegis' "$FASES/80-supply-chain.sh" | head -1 | cut -d: -f1)"
