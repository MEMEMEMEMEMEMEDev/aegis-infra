# titulo: build del directorio editado ANTES del commit (Patrón A-2c in-VM)
# origen: verify-static.sh (v2) ══ 56
check() {
# el error de una inyección/entry mala se veía 3 eslabones después:
L_KB="$(grep -n 'kustomize-build-policies' "$FASES/80-supply-chain.sh" | head -1 | cut -d: -f1)"
L_PK="$(grep -n 'gate "policy-en-kustomization"' "$FASES/80-supply-chain.sh" | head -1 | cut -d: -f1)"
# sesión 21 (clase F): el commit de la 80 ahora va por
# git_commit_if_changes — el ancla es ese helper, no el git crudo:
L_CM="$(grep -n 'git_commit_if_changes "\$PLATFORM_DIR"' "$FASES/80-supply-chain.sh" | head -1 | cut -d: -f1)"
