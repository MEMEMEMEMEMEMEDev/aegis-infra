# titulo: secretos NUNCA por argv de curl + bordes (W-03 / SEC-12/SEC-02)
...
grep -q -- '-K "\$cfg"' "$FASES/15-terceros.sh" \
    || D75="$D75 _cf_call no usa -K (config de curl por archivo);"
grep -q -- '-K "\$_cf_cfg"' "$FASES/25-edge-tofu.sh" \
...
grep -q 'chmod 600 "\$f"' "$FASES/25-edge-tofu.sh" \
