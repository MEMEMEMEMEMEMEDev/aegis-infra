...
#      own verdict (BAD:/GOOD:/NOTE:/NOT EVALUATED) and bash distributes
...
#      ONCE, in `dispatch`, with the guard inside. This check maintains
...
#      is.)
#
#   3. degradation in grey. `note()` counts nothing, and that is right
...
#      verdict never finds out. That is what `degraded()` is for: it
...
if [[ ! -f "$CHQ" ]]; then
    ...
    N_CASE="$(grep -c 'BAD:\*)' "$CHQ" || true)"
    [[ "$N_CASE" == "1" ]] \
        || D93="$D93 there are $N_CASE dispatches of BAD:/GOOD: lines (there must be ONE, inside dispatch): a copy of the protocol is a copy of the hole;"
    grep -q '^dispatch() {' "$CHQ" \
        || D93="$D93 the dispatch function is missing: without it every big meter distributes its lines by hand again;"
    # and the guard that makes it count: without the «it said nothing»
    # branch, dispatch is the same old case under another name.
    # The CONDITION is what is searched for, not the word: `grep said`
    # passed green with the guard neutralized to `true ||`, because the
    # `said=1` assignments were still there. A check satisfied by the
    # name appearing does not measure the guard, it measures the
    # spelling.
    sed -n '/^dispatch() {/,/^}/p' "$CHQ" | grep -qE '\(\(\s*said\s*\)\)\s*\|\|\s*notice' \
        || D93="$D93 dispatch does not carry the «if it said nothing, it is a notice» guard — without that line it is the old case under another name;"
...
PY
    ...
    grep -q '^degraded() {.*notices=' "$CHQ" \
        || D93="$D93 degraded() is missing (or stopped adding notices): a weakened measurement would go back to being reported only in grey;"
    ...
    # right thing is to delete degraded(), not leave it dead — the same
    # rule as aegis dev seed's exclusions.
    N_DEG="$(grep -cE '^\s*degraded ' "$CHQ" || true)"
    [[ "$N_DEG" -ge 1 ]] \
        || D93="$D93 degraded() exists and nobody calls it: either degradation went back to being reported in grey, or the function is surplus;"
    grep -qE '^note\(\)\s+\{[^}]*notices=' "$CHQ" \
        && D93="$D93 note() started counting notices: then it no longer serves as a detail line of a failure/notice and every detail inflates the tally;"
fi
if [[ -n "$D93" ]]; then fail "the round can fall silent:$D93"
else pass "the round has no way of falling silent: a single dispatch with its guard, no except that exits with success, and degradation counts ($N_DEG use(s)) while note() still does not count"; fi
}
