...
nc "$FASES/00-preflight.sh" | grep -q 'check_domain_on_cloudflare' \
...
L_DEPS="$(nc "$FASES/05-host.sh" | grep -n 'python3-yaml' | head -1 | cut -d: -f1)"
L_LOOP="$(nc "$FASES/05-host.sh" | grep -n '^for t in jq git' | head -1 | cut -d: -f1)"
