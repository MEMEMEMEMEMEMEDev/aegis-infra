# dientes del check 066 — generados el 2026-08-23 y VERIFICADOS:
# cada rojo se aplicó sobre una copia del árbol y el check se puso rojo.

# saca del artefacto justo lo que el check dice medir
red_1() {
    grep -vE 'secretName.*registry-tls' "$AEGIS_ROOT/init/phases/40-registry-pki.sh" > "$AEGIS_ROOT/init/phases/40-registry-pki.sh.diente" \
