# dientes del check 075 — generados el 2026-08-23 y VERIFICADOS:
# cada rojo se aplicó sobre una copia del árbol y el check se puso rojo.

# saca del artefacto justo lo que el check dice medir
rojo_1() {
    grep -vE 'trap '\''secrets_cleanup; exit 130'\'' INT TERM' "$AEGIS_ROOT/lib/secrets.sh" > "$AEGIS_ROOT/lib/secrets.sh.diente" \
